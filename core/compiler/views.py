...
#         result = run_code(language, code, stdin_input)
...
#     print(f"[DEBUG] Code written to {code_file_path}")
#     # Write input
#     with open(input_file_path, "w") as input_file:
#         input_file.write(input_data)
#     print(f"[DEBUG] Input written to {input_file_path}: {repr(input_data)}")
...
#         if compile_result.returncode != 0:
#                 return {
#                     "output": clean_error_output(compile_result.stderr),
#                     "uuid": unique,
#                     "status": "Error"
#                 }
...
def run_code(language, code, input_data):
    ...
    try:
        if language == "cpp":
            ...
            if compile_result.returncode != 0:
                return {'error': compile_result.stderr.decode(), 'uuid': unique}
            ...
            return {"error": f"Language '{language}' not supported.", 'uuid': unique}

    except subprocess.TimeoutExpired:
        return {"error": "Time Limit Exceeded", 'uuid': unique}
    except Exception as e:
        return {"error": str(e), 'uuid': unique}
